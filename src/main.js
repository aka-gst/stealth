...
if (location.search.includes('debug') || capture) {
    window.perimetr = {
        ...
        scene(opts = {}) {
            ...
            // Витринный режим для отрисовки: конус зрения рисуется плотнее,
            // чем в игре. Слова Сергея 6 сентября — показать стража «вместе
            // со своим конусом зрения»; в игре конус нарочно бледный (4.5%
            // прозрачности), и на полосе карточки в 271 точку от него не
            // остаётся ничего. Флаг ставит сцена и только сцена — в самой
            // игре яркость не меняется.
            world.витрина = true;

            let t = 0;
            let struck = false;
            let замах = 0;
            // Просьба Сергея 6 сентября: «враг стоит, немножко влево-вправо
            // смотрит — вместе со своим конусом зрения». Конус игра рисует
            // сама, нужно только качать взгляд; амплитуда мала нарочно, иначе
            // страж поворачивается к герою и сцена перестаёт быть заходом
            // со спины.
            ...
            scenePlay = (dt) => {
                ...
                // Медленный подход — просьба Сергея 6 сентября: «должен
                // МЕДЛЕННО подходить со спины». Прежде герой доходил за
                // полсекунды, и намерение не читалось. Отодвинуть старт
                // нельзя — дальше по коридору он упирается и не идёт вовсе;
                // поэтому замедляем сам шаг и даём больше времени.
                ...
                // Летально — по слову Сергея 6 сентября: «убит красным».
                // Это суть игры, а не украшение: тебя убивают с одного касания,
                // их тоже. Нелетальное снятие даёт жёлтое «СНЯТ» и в кадре
                // читается слабее.
                // Замах перед ударом — просьба Сергея 6 сентября: «должна быть
                // АНИМАЦИЯ убийства». В самой игре её нет, снятие мгновенное;
                // здесь минимальная сценическая: подойдя вплотную, герой
                // замирает на четыре кадра и только потом бьёт. Без этого
                // страж переходил из «стоит» в «труп» между двумя кадрами, и
                // удара в петле не было видно вовсе.
                if (!struck && d < 30) {
                    // Удар ножом растянут во времени: «АНИМАЦИЯ НОЖОМ
                    // убийства» — просьба Сергея. Пока идёт замах, герой стоит
                    // за спиной, а рендер рисует лезвие (world.нож — доля от 0
                    // до 1, по ней же считается блик).
                    замах += dt;
                    world.нож = Math.min(1, замах / 0.5);
                    if (замах >= 0.5) { struck = tryTakedown(world, true); world.нож = 0; }
                }
                world.hint = '';
                if (zoom) {
                    renderer.focus = {
